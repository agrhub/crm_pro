...
public class AdminRoleController {
    ...
    public Result<List<Map<String, Object>>> getRoleTypeList() {
        ...
        data.add(new JSONObject().fluentPut("name", "System Management").fluentPut("roleType", 1));
        data.add(new JSONObject().fluentPut("name", "Office Management").fluentPut("roleType", 7));
        data.add(new JSONObject().fluentPut("name", "Customer Management").fluentPut("roleType", 2));
        data.add(new JSONObject().fluentPut("name", "Project Management").fluentPut("roleType", 8));
        return R.ok(data);
    }
    ...
    @PostMapping("/add")
    @ApiOperation("添加角色")
    @SysLogHandler(behavior = BehaviorEnum.SAVE,object = "#adminRole.roleName",detail = "'Role added:'+#adminRole.roleName")
