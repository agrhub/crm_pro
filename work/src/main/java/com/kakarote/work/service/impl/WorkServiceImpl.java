...
//            throw new CrmException(WorkCodeEnum.WORK_CREATE_NAME_NULL_ERROR);
    ...
    public List<WorkTaskTemplateClassVO> queryTaskByWorkId(WorkTaskTemplateBO workTaskTemplateBO) {
        ...
        item.setClassId(-1).setClassName("(Unclassified)");
    ...
    public Dict excelImport(MultipartFile file, Integer workId) throws IOException {
        ...
        ExcelUtil.readBySax(file.getInputStream(), 0, (int sheetIndex, int rowIndex, List<Object> rowList) -> {
            if (rowIndex > 1001) {
                rowList.add(0, "Import up to 1000 data at the same time");
                errList.add(rowList);
                return;
            }
            if (rowIndex > 1) {
                ...
                if (StrUtil.isEmptyIfStr(rowList.get(0))) {
                    rowList.add(0, "The task name cannot be empty");
                    errList.add(rowList);
                    return;
                }
                if (!StrUtil.isEmptyIfStr(rowList.get(2))) {
                    ...
                    if (!BaseUtil.isTime(time)) {
                        rowList.add(0, "The start time and date format is wrong, for example: 2000-01-01");
                        errList.add(rowList);
                        return;
                    }
                }
                if (!StrUtil.isEmptyIfStr(rowList.get(3))) {
                    ...
                    if (!BaseUtil.isTime(time)) {
                        rowList.add(0, "The end time and date format is wrong, for example: 2000-01-01");
                        errList.add(rowList);
                        return;
                    }
                }
                if (StrUtil.isEmptyIfStr(rowList.get(6))) {
                    rowList.add(0, "Owning task list cannot be empty");
                    errList.add(rowList);
                    return;
                }
                ...
                if (StrUtil.isNotEmpty(mainUserName)) {
                    if (ObjectUtil.isEmpty(workUserMap.get(mainUserName))) {
                        rowList.add(0, "The manager does not match the current project member");
                        errList.add(rowList);
                        return;
                    } else {
                ...
                if (StrUtil.isNotEmpty(ownerUserName)) {
                    ...
                    for (String userName : ownerUserNameList) {
                        if (ObjectUtil.isEmpty(workUserMap.get(userName))) {
                            rowList.add(0, "The member does not match the current project member");
                            errList.add(rowList);
                            return;
                        }
                    }
        ...
        if (errList.size() > 0) {
            ...
            try {
                ...
                writer.merge(7, "Task import template (*) is required");
                for (int i = 0; i < 8; i++) {
                ...
                writer.writeHeadRow(Arrays.asList("Error Message", "Task Name (*)", "Task Description", "Start Time", "End Time", "Manager", "Participants", "List of Tasks (*)"));
    ...
    public List<Map<String, Object>> workTaskExport(Integer workId) {
        ...
        recordList.forEach(record -> {
            ...
            if (record.get("contactsIds") != null) {
                ...
                if (ids.size() > 0) {
                    relateCrmWork.append("Contact person ");
