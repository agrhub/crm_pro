...
public class CrmProductServiceImpl extends BaseServiceImpl<CrmProductMapper, CrmProduct> implements ICrmProductService, CrmPageService {
    ...
    public List<CrmModelFiledVO> queryField(Integer id) {
        ...
        crmModelFiledVOS.add(new CrmModelFiledVO("status", FieldEnum.SELECT, "productStatus", 1).setIsNull(1).setSetting(statusList).setValue(crmModel.get("status")).setAuthLevel(authLevel));
    ...
    public BasePage<Map<String, Object>> queryPageList(CrmSearchBO search) {
        ...
        basePage.getList().forEach(map -> {
            ...
            map.put("status", Objects.equals("1", status) ? "on" : "off");
        });
        return basePage;
    }
...
//            crmProductDataService.deleteByBatchId(batchIdList);
    ...
    public void downloadExcel(HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            log.error("Download product template error:", e);
        } finally {
            wb.close();
        }
    }
    ...
    public void exportExcel(HttpServletResponse response, CrmSearchBO search) {
        ...
        try (ExcelWriter writer = ExcelUtil.getWriter()) {
            ...
            log.error("Export product error:", e);
        }
    }
    ...
    public List<CrmModelFiledVO> information(Integer productId) {
        ...
        String status = Objects.equals(1, crmModel.get("status")) ? "on" : "off";
        crmModelFiledVOS.add(new CrmModelFiledVO("status", FieldEnum.SELECT, "productStatus", 1).setValue(status).setAuthLevel(authLevel));
