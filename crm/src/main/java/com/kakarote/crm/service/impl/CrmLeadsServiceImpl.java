...
public class CrmLeadsServiceImpl extends BaseServiceImpl<CrmLeadsMapper, CrmLeads> implements ICrmLeadsService, CrmPageService {
        ...
        for (Integer leadsId : leadsIds) {
            ...
            for (CrmField leadsField : leadsFields) {
                for (CrmField customerField : customerFields) {
                    ...
                    boolean bol = ("customerSource".equals(customerField.getName()) && "leadsSource".equals(leadsField.getName()))
                            || ("customerIndustry".equals(customerField.getName()) && "customerIndustry".equals(leadsField.getName()))
                            || ("customerLevel".equals(customerField.getName()) && "customerLevel".equals(leadsField.getName()));
                    if (bol) {
    ...
    public void downloadExcel(HttpServletResponse response) throws IOException {
        ...
        HSSFSheet sheet = wb.createSheet("Lead import form");
        ...
        try {
            ...
            log.error("downloadExcel error: ", e);
        } finally {
            wb.close();
        }
    }
    ...
    public void exportExcel(HttpServletResponse response, CrmSearchBO search) {
        ...
        try (ExcelWriter writer = ExcelUtil.getWriter()) {
            ...
            writer.merge(headList.size() - 1, "Leads information");
            if (dataList.size() == 0) {
    ...
    public List<CrmModelFiledVO> information(Integer leadsId) {
        ...
        List<String> systemFieldList = Arrays.asList("createdBy", "createdTime", "updatedTime", "lastFollowupTime", "lastFollowupRecord");
    ...
    public List<FileEntity> queryFileList(Integer leadsId) {
        ...
        if (crmFields.size() > 0) {
            ...
            data.forEach(fileEntity -> {
                fileEntity.setSource("Lead details");
