...
//            }
    ...
    public void exportExcel(HttpServletResponse response, CrmSearchBO search) {
        ...
        try (ExcelWriter writer = ExcelUtil.getWriter()) {
            ...
            log.error("Export contract error:", e);
        }
    }
    ...
    public List<CrmMembersSelectVO> getMembers(Integer contractId) {
        ...
        if (crmContract.getOwnerUserId() != null) {
            ...
            crmMembersVO.setPower("owner");
            crmMembersVO.setGroupRole("ownerUserName");
        ...
        for (String memberId : memberIdsSet) {
            ...
            if (roUserId.contains(memberId)) {
                crmMembersVO.setPower("Read only");
            } else if (rwUserId.contains(memberId)) {
                crmMembersVO.setPower("Read and write");
            }
            recordList.add(crmMembersVO);
        }
        return recordList;
    ...
    public List<CrmModelFiledVO> information(Integer contractId) {
        ...
        List<String> systemFieldList = Arrays.asList("createdBy", "createdTime", "updatedTime", "nextContactTime", "lastFollowUpTime", "receivedMoney", "unreceivedMoney");
        List<CrmModelFiledVO> crmModelFiledVOS = queryInformation(crmModel, keyList);
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("checkStatus").setName("checkStatus").setValue(crmModel.get("checkStatus")).setFormType("check_status").setFieldType(1));
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("businessId").setName("businessName").setValue(new JSONObject().fluentPut("businessId", crmModel.get("businessId")).fluentPut("businessName", crmModel.get("businessName"))).setFormType("business").setFieldType(1));
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("contactsId").setName("contactsName").setValue(new JSONObject().fluentPut("contactsId", crmModel.get("contactsId")).fluentPut("contactsName", crmModel.get("contactsName"))).setFormType("contacts").setFieldType(1));
        ...
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("companyUserId").setName("companyUserId").setValue(ids.size() > 0 ? adminService.queryUserByIds(ids).getData() : new ArrayList<>()).setFormType("user").setFieldType(1));
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("customerId").setName("customerName").setValue(new JSONObject().fluentPut("customerId", crmModel.get("customerId")).fluentPut("customerName", crmModel.get("customerName"))).setFormType("customer").setFieldType(1));
        if (crmModel.get("receivedMoney") != null){
            crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("receivedMoney").setName("receivedMoney").setValue(new BigDecimal(crmModel.get("receivedMoney").toString())).setFormType(FieldEnum.NUMBER.getFormType()).setFieldType(1));
        }
        if (crmModel.get("unreceivedMoney") != null){
            crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("unreceivedMoney").setName("unreceivedMoney").setValue(new BigDecimal(crmModel.get("unreceivedMoney").toString())).setFormType(FieldEnum.NUMBER.getFormType()).setFieldType(1));
        }
    ...
    public JSONObject queryProductListByContractId(CrmRelationPageBO crmRelationPageBO) {
        ...
        for (JSONObject jsonObject : page.getList()) {
            ...
            if (status == 1) {
                jsonObject.put("productStatus", "on");
            } else {
                jsonObject.put("productStatus", "off");
            }
        }
    ...
    public List<FileEntity> queryFileList(Integer contractId) {
        ...
        adminFileService.queryFileList(crmContract.getBatchId()).getData().forEach(fileEntity -> {
            fileEntity.setSource("Updating files");
