...
public interface CrmPageService {
            ...
            if (crmScene != null) {
                if (StrUtil.isNotEmpty(crmScene.getBydata())) {
                    ...
                    try {
                        ...
                        log.error("json serialization error{}", crmScene.getData());
    ...
    default void sort(CrmSearchBO crmSearchBO, SearchSourceBuilder sourceBuilder) {
        ...
        crmFieldSortList.add(new CrmFieldSortVO().setFieldName("receiveTime").setName("receiveTime").setType(FieldEnum.DATETIME.getType()));
        crmFieldSortList.add(new CrmFieldSortVO().setFieldName("preOwnerUserName").setName("preOwnerUserName").setType(FieldEnum.TEXT.getType()));
        crmFieldSortList.add(new CrmFieldSortVO().setFieldName("createTime").setName("createTime").setType(FieldEnum.DATETIME.getType()));
        crmFieldSortList.add(new CrmFieldSortVO().setFieldName("lastTime").setName("lastTime").setType(FieldEnum.DATETIME.getType()));
        crmFieldSortList.add(new CrmFieldSortVO().setFieldName("poolTime").setName("poolTime").setType(FieldEnum.DATETIME.getType()));
        if (crmSearchBO.getPage() <= 100) {
    ...
    default void updateField(String fieldName, Object value, List<Integer> ids) {
        ...
        try {
            ...
            log.error("es modification failed", e);
        }
    }
    ...
    public List<CrmModelFiledVO> queryInformation(CrmModel crmModel, List<String> fieldKey) {
        ...
        if (!getLabel().equals(CrmEnum.RETURN_VISIT)) {
            ...
            filedVO.setName("owner username");
        ...
        if (getLabel().equals(CrmEnum.CUSTOMER)) {
            CrmModelFiledVO filedVO = new CrmModelFiledVO("owner_user_name", FieldEnum.SINGLE_USER, "owner username", 1);
            ...
            filedVOS.add(filedVO);
            filedVOS.add(new CrmModelFiledVO("receive_time", FieldEnum.DATETIME, "receive time", 1).setValue(crmModel.get("receiveTime")));
        }
        if (getLabel().equals(CrmEnum.CUSTOMER) || getLabel().equals(CrmEnum.LEADS)) {
            filedVOS.add(new CrmModelFiledVO("last_content", FieldEnum.TEXTAREA, "last content", 1).setValue(crmModel.get("lastContent")));
        }
        Object value = adminService.queryUserByIds(Collections.singletonList((Long) crmModel.get("createUserId"))).getData();
        filedVOS.add(new CrmModelFiledVO("create_user_name", FieldEnum.USER, "create username", 1).setValue(value));
        ...
        if (!getLabel().equals(CrmEnum.PRODUCT) && !getLabel().equals(CrmEnum.RECEIVABLES) && !getLabel().equals(CrmEnum.RETURN_VISIT)) {
            filedVOS.add(new CrmModelFiledVO("last_time", FieldEnum.DATETIME, "last time", 1).setValue(crmModel.get("lastTime")));
        }
        return filedVOS;
    }
