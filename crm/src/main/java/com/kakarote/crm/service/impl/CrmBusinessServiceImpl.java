...
public class CrmBusinessServiceImpl extends BaseServiceImpl<CrmBusinessMapper, CrmBusiness> implements CrmPageService, ICrmBusinessService {
    ...
    public List<CrmModelFiledVO> queryField(Integer id) {
        ...
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("type_id").setName("type_id").setValue(crmModel.get("type_id")).setFormType("business_type").setSetting(new ArrayList<>()).setIsNull(1).setFieldType(1).setValue(crmModel.get("typeId")).setAuthLevel(3));
        Object statusId = crmModel.get("statusId");
        if(!Objects.equals(0,crmModel.get("isEnd"))){
            statusId = crmModel.get("isEnd");
        }
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("status_id").setName("status_id").setValue(crmModel.get("status_id")).setFormType("business_status").setSetting(new ArrayList<>()).setIsNull(1).setFieldType(1).setValue(statusId).setAuthLevel(3));
    ...
    public List<CrmModelFiledVO> information(Integer businessId) {
        ...
        List<String> systemFieldList = Arrays.asList("createdBy", "createdTime", "updatedTime", "nextContactTime", "lastFollowUpTime", "amountReceived", "amountNotReceived");
        List<CrmModelFiledVO> crmModelFiledVOS = queryInformation(crmModel, keyList);
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("statusName").setName("statusName").setValue(new JSONObject().fluentPut("statusId", crmModel.get("statusId")).fluentPut("statusName", crmModel.get("statusName"))).setFormType("statusName").setFieldType(1));
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("typeName").setName("typeName").setValue(new JSONObject().fluentPut("typeId", crmModel.get("typeId")).fluentPut("typeName", crmModel.get("typeName"))).setFormType("typeName").setFieldType(1));
        crmModelFiledVOS.add(new CrmModelFiledVO().setFieldName("customerId").setName("customerId").setValue(new JSONObject().fluentPut("customerId", crmModel.get("customerId")).fluentPut("customerName", crmModel.get("customerName"))).setFormType("customer").setFieldType(1));
    ...
    public void exportExcel(HttpServletResponse response, CrmSearchBO search) {
        ...
        try (ExcelWriter writer = ExcelUtil.getWriter()) {
            ...
            log.error("Export opportunity error:", e);
        }
    }
