...
public class CrmReceivablesPlanServiceImpl extends BaseServiceImpl<CrmReceivablesPlanMapper, CrmReceivablesPlan> implements ICrmReceivablesPlanService {
    ...
    public List<CrmModelFiledVO> queryField(Integer id) {
        ...
        fieldList.add(new CrmModelFiledVO("customer_id", FieldEnum.CUSTOMER,1).setName("customerName").setIsNull(1));
        fieldList.add(new CrmModelFiledVO("contract_id", FieldEnum.CONTRACT,1).setName("contractNo").setIsNull(1));
        fieldList.add(new CrmModelFiledVO("money", FieldEnum.NUMBER,1).setName("amount").setIsNull(1));
        fieldList.add(new CrmModelFiledVO("return_date", FieldEnum.DATE,1).setName("paymentDate").setIsNull(1));
        fieldList.add(new CrmModelFiledVO("return_type", FieldEnum.SELECT,1).setName("paymentMethod").setIsNull(1));
        fieldList.add(new CrmModelFiledVO("remind", FieldEnum.NUMBER,1).setName("remind").setIsNull(0));
        fieldList.add(new CrmModelFiledVO("remark", FieldEnum.TEXTAREA,1).setName("remark").setIsNull(0));
        fieldList.forEach(field -> {
            field.setAuthLevel(3);
            if ("returnType".equals(field.getFieldName())) {
                field.setValue(map.get(field.getFieldName()));
                field.setSetting(Arrays.asList("check", "cash", "postalRemittance", "wireTransfer", "onlineTransfer", "momo", "zaloPay", "others"));
                return;
            }
