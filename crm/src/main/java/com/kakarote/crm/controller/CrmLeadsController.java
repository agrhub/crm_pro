...
public class CrmLeadsController {
    ...
    public Result<CrmModel> queryById(@PathVariable("leadsId") @ApiParam(name = "id", value = "id") Integer leadsId) {
        ...
        if (number == 0){
            throw new CrmException(CrmCodeEnum.CRM_DATA_DELETED,"Leads");
        }
    ...
    @ApiOperation("全部导出")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_EXPORT,object = "线索导出",detail = "Export leads")
    ...
    @PostMapping("/add")
    @ApiOperation("保存数据")
    @SysLogHandler(behavior = BehaviorEnum.SAVE,object = "#crmModel.entity[leadsName]",detail = "'Added leads:' + #crmModel.entity[leadsName]")
    ...
    @ApiOperation("导入线索")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_IMPORT,object = "导入线索",detail = "Import leads")
