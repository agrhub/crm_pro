...
public class CrmReceivablesController {
    ...
    @PostMapping("/add")
    @ApiOperation("保存数据")
    @SysLogHandler(behavior = BehaviorEnum.SAVE,object = "#crmModel.entity.[number]",detail = "'Added payment:' + #crmModel.entity[number]")
    ...
    public Result<CrmModel> queryById(@PathVariable("receivablesId") @ApiParam(name = "id", value = "id") Integer receivablesId) {
        ...
        if (number == 0){
            throw new CrmException(CrmCodeEnum.CRM_DATA_DELETED,"Payment");
        }
    ...
    @ApiOperation("选中导出")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_EXPORT,object = "回款导出",detail = "Export payment")
