...
public class CrmContactsController {
    ...
    public Result<CrmModel> queryById(@PathVariable("contactsId") @ApiParam(name = "id", value = "id") Integer contactsId) {
        ...
        if (number == 0){
            throw new CrmException(CrmCodeEnum.CRM_DATA_DELETED,"Contact person");
        }
    ...
    @ApiOperation("选中导出")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_EXPORT,object = "选中导出",detail = "Export contacts")
    ...
    @ApiOperation("全部导出")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_EXPORT,object = "全部导出",detail = "Export contacts")
    ...
    @PostMapping("/add")
    @ApiOperation("保存数据")
    @SysLogHandler(behavior = BehaviorEnum.SAVE, object = "#crmModel.entity[name]", detail = "'New contact:' + #crmModel.entity[name]")
    ...
    @ApiOperation("导入联系人")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_IMPORT,object = "导入联系人",detail = "Import contacts")
