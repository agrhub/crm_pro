...
public class OaLogController {
    ...
    @PostMapping("/export")
    @ApiOperation("导出")
    @SysLogHandler(behavior = BehaviorEnum.EXCEL_EXPORT,object = "导出日志",detail = "Export log")
    public void export(@RequestBody LogBO logBO,HttpServletResponse response){
        ...
        try (ExcelWriter writer = ExcelUtil.getWriter()) {
            writer.addHeaderAlias("category", "Log type");
            writer.addHeaderAlias("createTime", "Creation date");
            writer.addHeaderAlias("createUserName", "Employee");
            writer.addHeaderAlias("sendName", "Manager");
            writer.addHeaderAlias("content", "Today's work content");
            writer.addHeaderAlias("tomorrow", "Tomorrow's work content");
            writer.addHeaderAlias("question", "Encounter problems");
            writer.addHeaderAlias("relateCrmWork", "Related business");
            writer.addHeaderAlias("comment", "Comment");
            writer.merge(8, "Log information");
    ...
    @SysLogHandler(applicationName = "admin",subModel = SubModelType.ADMIN_OTHER_SETTINGS,behavior = BehaviorEnum.UPDATE,object = "设置日志提交设置",detail = "Set log submission settings")
